    ...
    <div className="group relative overflow-hidden rounded-3xl border border-border/50 bg-card/90 shadow-md backdrop-blur transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/20 hover:border-primary/30 active:translate-y-0 focus-within:border-primary/40 pk-glass">
      ...
      <div className="flex flex-col gap-3 p-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-muted/80 px-2.5 py-1.5 text-xs font-medium text-muted-foreground shadow-sm">
            <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
            <span className="font-bold text-foreground">{product.rating.toFixed(1)}</span>
            <span className="text-muted-foreground">({product.reviews})</span>
          </div>
          {!product.inStock && (
            <span className="rounded-full bg-destructive/15 px-2.5 py-1 text-xs font-bold text-destructive">Out of stock</span>
          )}
        </div>
        ...
        <button
          ...
          className={cn(
            'pk-btn pk-btn-primary pk-btn-shine h-11 px-4 text-sm font-semibold shadow-lg transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0',
